...
A GitHub Actions workflow that runs every 5 minutes, creates a file named with the
...
- Trigger: `schedule` (cron `*/5 * * * *`) and manual `workflow_dispatch`
...
> during periods of high load, so runs may not be exactly 5 minutes apart.
